...
CREATE TABLE `__new_invoices` (
    ...
    ("__new_invoices"."status" = 'draft' and "__new_invoices"."due_date" is null)
    or
    ("__new_invoices"."status" in ('issued', 'voided') and "__new_invoices"."due_date" is not null)
  ),
...
INSERT INTO `__new_invoices`("id", "customer_id", "invoice_number", "status", "issued_at", "due_date", "voided_at", "created_at", "updated_at") SELECT "id", "customer_id", "invoice_number", "status", "issued_at", "due_date", "voided_at", "created_at", "updated_at" FROM `invoices`;--> statement-breakpoint
