...
CREATE UNIQUE INDEX `invoice_jobs_active_job_unique` ON `invoice_jobs` (`job_id`) WHERE "invoice_jobs"."released_at" is null;--> statement-breakpoint
...
--> statement-breakpoint
INSERT INTO `__new_invoices`("id", "customer_id", "invoice_number", "status", "issued_at", "voided_at", "created_at", "updated_at") SELECT "id", "customer_id", "invoice_number", "status", "issued_at", "voided_at", "created_at", "updated_at" FROM `invoices`;--> statement-breakpoint
...
ALTER TABLE `invoice_items` ADD `job_id` text NOT NULL REFERENCES jobs(id);--> statement-breakpoint
