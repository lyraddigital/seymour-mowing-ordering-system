...
import { OutlineInput, OutlinePasswordInput } from "@/app/core/components/ui/forms/inputs";
...
export default function SignInForm() {
    ...
    return (
        <Box component="form" action={action} noValidate autoComplete="off" sx={{ m: 1 }}>
            <FormActionAlerts error={state?.error}></FormActionAlerts>
            <Box sx={{mb: 1}}>
                <OutlineInput fieldName={formFields.username} errors={state?.error} label="Username *" defaultValue={state?.data?.username} />
            </Box>
            <Box sx={{mb: 1}}>
                <OutlinePasswordInput fieldName={formFields.password} errors={state?.error} defaultValue={state?.data?.password} />
            </Box>
            <Box sx={{mt: 2, mb: 3}}>
