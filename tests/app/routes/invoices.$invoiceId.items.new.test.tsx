...
it("renders the item form with job context and validation errors", async () => {
  ...
  const router = createMemoryRouter([
    {
      ...
      element: (
        <AddInvoiceItemPage
          {...result}
          values={{ jobId: fixture.jobId, description: "Retry", amount: "-1" }}
  ...
  expect(html).toContain('type="hidden" name="jobId"');
});

it("offers only the selected jobs for a multi-job invoice", async () => {
  await updateDraftInvoiceJobs(env.DB, fixture.admin, fixture.invoiceId, {
    jobIds: [fixture.jobId, fixture.otherJobId],
  });
  const result = await loader(args());
  const router = createMemoryRouter([
    { path: "/", element: <AddInvoiceItemPage {...result} /> },
  ]);
  const html = renderToStaticMarkup(<RouterProvider router={router} />);
