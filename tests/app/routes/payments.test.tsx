...
it.each(["admin", "operator"] as const)(
  ...
  async (role) => {
    ...
    const active = await recordPayment(
      ...
      fixture.admin,
      fixture.invoiceId,
      { amountCents: 1234, paymentDate: "2026-09-23" },
    );
    const voided = await recordPayment(
      env.DB,
      fixture.admin,
      fixture.invoiceId,
      { amountCents: 2500, paymentDate: "2026-09-24" },
    );
    ...
    for (const payment of result.payments) {
      expect(html).toContain(`dateTime="${payment.paymentDate}"`);
    ...
    expect(
      result.payments.find((payment) => payment.id === active.id)?.voidedAt,
    ).toBeNull();
    expect(
      result.payments.find((payment) => payment.id === voided.id)?.voidedAt,
    ).not.toBeNull();
...
it("renders payments in payment-date order", async () => {
...
it("keeps voided payments in payment history", async () => {
  const payment = await recordPayment(
    ...
    fixture.admin,
    fixture.invoiceId,
    { amountCents: 5000, paymentDate: "2026-09-24" },
  );
