...
import { action } from "../../../app/routes/invoices.$invoiceId.issue";
...
it("issues a draft invoice and redirects to the invoice", async () => {
...
it("keeps the job assigned after issue", async () => {
  const { id: invoiceId } = await createDraftInvoice(env.DB, admin, {
    jobIds: [jobId],
  });
