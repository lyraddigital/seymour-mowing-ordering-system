...
export default function DashboardSummaryTiles({ summary }: DashboardTilesProps) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-5 my-3">
      ...
      <DashboardSummaryTile className="hidden sm:block" title="Customers owing" color="red" value={summary.numberOfCustomersOwing.toString()} />
      <DashboardSummaryTile title="Amount recieved (last 30)" color="green" value={getCurrencyString(summary.amountReceivedLastThirty)} />
      <DashboardSummaryTile className="hidden sm:block" title="Amount recieved (FY 25)" color="green" value={getCurrencyString(summary.amountReceivedLastThirty)} />
      <DashboardSummaryTile className="hidden sm:block" title="Jobs pending" color="yellow" value={"1"} />
    </div>
  );
}
