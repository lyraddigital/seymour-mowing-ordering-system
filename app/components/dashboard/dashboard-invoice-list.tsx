                    ...
                    <li key={ui.invoiceNumber} className="gap-3 p-4 text-sm flex hover:bg-slate-200 border-b h-[77px]">
                        <div className="flex flex-1 justify-between gap-10">
                            <div className="flex flex-col flex-1 gap-1 overflow-hidden">
                                <span className="font-bold truncate">{ui.customerName} ({getCurrencyString(ui.total)})</span>
                                <span className="text-slate-500 truncate">Due: {convertFromISOToShortDate(ui.dueDate)}</span>
                            </div>
