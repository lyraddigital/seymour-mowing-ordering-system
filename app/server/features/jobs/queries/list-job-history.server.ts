...
export async function listJobHistory(binding: Env["DB"], user: CurrentUser) {
  ...
  return db
    ...
    .where(inArray(jobStatusHistory.status, ["completed", "cancelled"]))
    .orderBy(desc(jobStatusHistory.createdAt), asc(jobs.id));
}
